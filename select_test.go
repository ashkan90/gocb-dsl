...
func TestWorkaround(t *testing.T) {
	...
	log.Println(Select(nil).From("buckt").Where(""))
}
...
func TestRealWorldExample(t *testing.T) {
	statement := Select("*").
		...
		Join(
			Select("meta.fields.*, meta.score").
				From("categories").As("t").
				Let(NewAlias("meta", "search_meta(t)")).
				Where(
					X("meta.fields.status").Eq("1").
						And(
							X("SEARCH(t, { 'query': {...}})").And(X("meta.score").Gte("0.4")),
						),
				),
		).As("c3 ON c0.id = c3.parent").
		LeftJoin("categories").As("c1 ON c1.id = c0.parent").
		OrderBy(Desc("c3.score")).
		String()
...
func TestSelectAlternatives(t *testing.T) {
	var alternatives = []struct {
		...
		// No alias, raw select query.
		{
			Alternative: Select(S("field")),
			Output:      `SELECT "field"`,
		},
		{
			Alternative: Select("field"),
			Output:      `SELECT field`,
		},
		// With alias select query.
		{
			Alternative: Select(S("field").As("as_field")),
			Output:      `SELECT "field" AS as_field`,
		},
		{
			Alternative: Select("field as as_field"),
			Output:      `SELECT field as as_field`,
		},
		{
			Alternative: Select(Meta("bucket").As("meta")).From("bucket-name"),
			Output:      "SELECT META(bucket) AS meta FROM `bucket-name`",
		},
		// Nil field
		{
			Alternative: Select(nil),
			Output:      `SELECT <nil>`,
		},
	}

	for _, alternative := range alternatives {
		assert.Equal(t, alternative.Output, alternative.Alternative.String())
	}
}
...
func TestSelectAs(t *testing.T) {
	var statements = []struct {
		...
		{
			Alternative: Select(S("field").As("field_as")),
			Output:      `SELECT "field" AS field_as`,
		},
	}

	for _, statement := range statements {
		assert.Equal(t, statement.Output, statement.Alternative.String())
	}
}

func TestWhereAlternatives(t *testing.T) {
	var alternatives = []struct {
		Alternative Statement
		Output      string
	}{
		{
			Alternative: Select("*").From("bucket-name").Where(X("field").Eq(S("field_val"))),
			Output:      "SELECT * FROM `bucket-name` WHERE field = \"field_val\"",
		},
		{
			Alternative: Select("*").From("bucket-name").Where("field = " + `"field_val"`),
			Output:      "SELECT * FROM `bucket-name` WHERE field = \"field_val\"",
		},
		{
			Alternative: Select(X("children[0].field").As("as_field")).From("bucket-name").Where(X("field").Eq(S("eq_field"))),
			Output:      "SELECT children[0].field AS as_field FROM `bucket-name` WHERE field = \"eq_field\"",
		},
	}
